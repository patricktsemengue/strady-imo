...
import { HomeIcon, DashboardIcon, HelpIcon, UserIcon, SparklesIcon, FileTextIcon } from '../Icons';
...
const AppFooter = ({ user, onProfileClick, isAIAssistantPage = false }) => {
    ...
    return (
        <footer className={`bg-white/90 backdrop-blur-sm border-t-2 shadow-top ${isAIAssistantPage ? '' : 'sticky'} bottom-0 left-0 right-0 z-20 print-hidden`}>
            <nav className="max-w-4xl mx-auto flex justify-around p-2">
                ...
                <NavItem to="/aide" icon={<HelpIcon />} label="Aide" page={page} activeCondition={p => p === 'aide'} />
                {user ? (
