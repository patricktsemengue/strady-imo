...
CREATE TABLE public.feedbacks (
  ...
  comment TEXT,
  -- Métadonnées optionnelles que vous pourriez vouloir ajouter plus tard
  user_email TEXT,
  user_name TEXT
);
...
-- Cela vous permet de voir tous les feedbacks dans le dashboard Supabase
...
-- Ajoute une colonne 'deleted_at' à votre table 'feedbacks'
-- Adaptez 'feedbacks' si votre table a un autre nom.
ALTER TABLE feedbacks
ADD COLUMN deleted_at timestamptz DEFAULT NULL;

-- (Optionnel) Créez un index pour optimiser les requêtes
-- qui filtrent les lignes supprimées.
CREATE INDEX idx_feedbacks_deleted_at
ON feedbacks (deleted_at)
